...
import ZoomCell from '../ui/ZoomCell.jsx';
...
export default function RejectionTable({ rows, loading, error, onRetry, onEdit, onDelete }) {
  const scrollRef = useHorizontalWheelScroll();
  const editable = !!onEdit;
  const { widths, startResize } = useColumnWidths(DEFAULT_WIDTHS, scrollRef);
  const th = { padding: '5px 6px', fontSize: 9.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.02em', color: '#3d4b4b', background: '#f5c542', border: '1px solid #d9b93a', whiteSpace: 'nowrap', position: 'sticky', top: 0, cursor: 'pointer', userSelect: 'none' };
  ...
  // "error" dibedakan dari "kosong beneran" -- lihat EmptyErrorState.jsx.
  if (loading && rows.length === 0) return <TableSkeleton rows={8} columns={6} />;
